...
export default function Employees() {
  ...
  const filtered = useMemo(() => {
    ...
    if (text) {
      list = list.filter((u) =>
        [u.name, u.full_name, u.email].some((v) =>
        ...
        >
          <div>
            ...
            <div style={{ color: "var(--ink-600)", marginTop: 4 }}>
              قائمة بكل المستخدمين المعتمدين في النظام (مدير + موظفين).
            </div>
          </div>
      ...
      <section className="q-card" style={{ padding: 12 }}>
        ...
        >
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            <input
              className="input"
              placeholder="بحث باسم/بريد الموظف…"
              style={{ minWidth: 260 }}
              value={q}
      ...
      <section className="q-card" style={{ padding: 0 }}>
        {loading ? (
          ...
          <div style={{ padding: 16, color: "var(--ink-600)" }}>
            لا يوجد موظفون مطابقون لنتيجة البحث.
          </div>
        ) : (
