...
export default function CasesList() {
  ...
  function startEdit(row) { setEditing((s) => ({ ...s, [row.id]: { status: row.status || "open", next: normalizeForInput(row.next) } })); }
  function cancelEdit(id) { setEditing((s) => { const n = { ...s }; delete n[id]; return n; }); }
  function changeEdit(id, patch) { setEditing((s) => ({ ...s, [id]: { ...(s[id] || {}), ...patch } })); }
  ...
  async function saveRow(id) {
    ...
    setSavingId(id);
    try {
      await updateCaseMeta(id, { status: data.status });
      await load();
      cancelEdit(id);
  ...
  const filteredSorted = useMemo(() => {
    ...
    if (text) {
      list = list.filter((r) => {
        ...
        return [r.id, r.title, r.status, assignedText, r.court, r.case_number].some((v) => String(v || "").toLowerCase().includes(text));
      });
    }
    ...
    const toMs = (s) => (s ? new Date(String(s).includes("T") ? s : String(s).replace(" ", "T")).getTime() : 0);
    ...
    list.sort((a, b) => {
      ...
      switch (sort.by) {
        ...
        case "assignedTo": return String(displayAssigned(a) || "").localeCompare(String(displayAssigned(b) || ""), "ar") * dir;
        case "next": return (toMs(a.next) - toMs(b.next)) * dir;
        case "updatedAt":
        default: return ((toMs(a.updatedAt) || toMs(a.next)) - (toMs(b.updatedAt) || toMs(b.next))) * dir;
      }
    });
    return list;
  ...
  const pageRows = useMemo(() => { const start = (page - 1) * PAGE_SIZE; return filteredSorted.slice(start, start + PAGE_SIZE); }, [filteredSorted, page]);
                  ...
                  return (
                    <tr key={r.id}>
                      ...
                      <td style={{ textAlign: "left", whiteSpace: "nowrap" }}>
                        {!isEdit ? (
                          <>
                            <button className="btn btn-primary" onClick={() => nav(`/admin/cases/${encodeURIComponent(r.id)}`)}>فتح</button>
                            <button className="btn btn-ghost" style={{ marginInlineStart: 8 }} onClick={() => startEdit(r)}>تعديل</button>
                          </>
                        ) : (
                          <>
                            <button className="btn btn-primary" disabled={savingId === r.id} onClick={() => saveRow(r.id)}>{savingId === r.id ? "يحفظ…" : "حفظ"}</button>
                            <button className="btn btn-ghost" style={{ marginInlineStart: 8 }} onClick={() => cancelEdit(r.id)}>إلغاء</button>
                          </>
                        )}
                      </td>
                    </tr>
