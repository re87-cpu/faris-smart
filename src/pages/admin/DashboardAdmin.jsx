...
import {
  ...
  listMyTasks, toggleMyTask,
} from "../../mock/api.js";
import { getAuth } from "../../utils/auth.js";
import { seedTx, fmtMoney, financialTotals } from "../../data/financialSeed.js";
...
export default function DashboardAdmin() {
  ...
  const fin = financialTotals(seedTx);
  ...
  return (
    <div dir="rtl" style={{ display: "flex", flexDirection: "column", gap: 22 }}>
      ...
      <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
        <StatCorner value={loading ? "—" : counters.active} label="قضايا نشطة" />
        <StatCorner value={loading ? "—" : counters.closed} label="قضايا منتهية" />
        <StatCorner value={loading ? "—" : counters.sessionsThisWeek} label="جلسات هذا الأسبوع" />
      ...
      <div>
        ...
        <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
          <StatCorner value={`${fmtMoney(fin.totalIncome)} ر.س`} label="الإيرادات المحصّلة" color="#1E7A45" to="/admin/financial" />
          <StatCorner value={`${fmtMoney(fin.totalExpense)} ر.س`} label="المصروفات" color="#C0392B" to="/admin/financial" />
          <StatCorner value={`${fmtMoney(fin.totalDue)} ر.س`} label="مستحقات غير محصّلة" color="var(--color-accent-700)" to="/admin/financial" />
          <StatCorner value="استشارات" label="أكثر الخدمات طلبًا" to="/admin/financial" />
        </div>
      </div>
